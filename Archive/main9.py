...
# generated the trying of palm filtering but that not good so skipped
...
# If more than this many markers disappear at once, assume a palm/hand is
# covering the keyboard instead of intentional individual key presses.
MAX_SIMULTANEOUS_PRESSES = 5
...
def get_ordered_missing_buttons(missing_buttons):
    return [
        button_name
...
def draw_debug(
    ...
    if calibrated and palm_blocked:
        cv2.putText(
            debug,
            f"hand/palm ignored: too many covered keys > {MAX_SIMULTANEOUS_PRESSES}",
            (20, 160),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.75,
            (0, 165, 255),
    ...
    elif calibrated and active_missing_starts:
        now = time.time()
        active_labels = []

        for button_name in get_ordered_missing_buttons(active_missing_starts):
            button = BUTTON_BY_NAME[button_name]
            elapsed = now - active_missing_starts[button_name]
            press_status = "pressed" if button_name in already_pressed else "timing"
            active_labels.append(f"{button.label} {elapsed:.2f}s {press_status}")

        cv2.putText(
            debug,
            "missing: " + ", ".join(active_labels[:3]),
            (20, 160),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.75,
            (0, 0, 255),
    ...
    try:
        for frame in mjpeg_frames(proc):
            ...
            if not calibrated:
                ...
                missing_buttons = set(BUTTON_BY_NAME.keys()) - visible_buttons
                palm_blocked = len(missing_buttons) > MAX_SIMULTANEOUS_PRESSES
                ...
                if palm_blocked:
                    ...
                    if now - last_status_print >= 1.0:
                        print(
                            f"Ignoring {len(missing_buttons)} covered markers. "
                            "Looks like a hand/palm, not individual key presses."
                        )
                        last_status_print = now

                else:
                    for button_name in list(active_missing_starts):
                        if button_name not in missing_buttons:
                            del active_missing_starts[button_name]

                    for button_name in missing_buttons:
                        active_missing_starts.setdefault(button_name, now)

                    for button_name in get_ordered_missing_buttons(missing_buttons):
                        missing_time = now - active_missing_starts[button_name]

                        if missing_time >= PRESS_SECONDS and button_name not in already_pressed:
                            button = BUTTON_BY_NAME[button_name]
                            print(f"Pressed {button.label}. Missing for {missing_time:.2f}s.")
                            caps_lock_on = press_button(ui, button, caps_lock_on)
                            already_pressed.add(button_name)
            ...
            debug = draw_debug(
                ...
                active_missing_starts=active_missing_starts,
                palm_blocked=palm_blocked,
